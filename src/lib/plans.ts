...
export const PLANS: Record<
  PlanKey,
  {
    ...
    /** Ile kont Google Calendar można podłączyć. null = bez limitu. */
    maxGoogleCalendars: number | null;
    priceMonthly: number | null;
  }
> = {
  START: { label: "Start", halls: "1 sala", maxAdmins: 1, maxUsers: 3, maxGoogleCalendars: 5, priceMonthly: 249 },
  PRO: { label: "Pro", halls: "do 5 sal", maxAdmins: 2, maxUsers: 10, maxGoogleCalendars: 10, priceMonthly: 599 },
  ENTERPRISE: { label: "Enterprise", halls: "powyżej 5 sal", maxAdmins: null, maxUsers: null, maxGoogleCalendars: null, priceMonthly: null },
};
...
export function effectiveLimits(
  ...
  overrides?: { maxAdmins?: number | null; maxUsers?: number | null; maxGoogleCalendars?: number | null },
): { maxAdmins: number | null; maxUsers: number | null; maxGoogleCalendars: number | null } {
  const p = PLANS[normalizePlan(plan)];
  return {
    maxAdmins: overrides?.maxAdmins ?? p.maxAdmins,
    maxUsers: overrides?.maxUsers ?? p.maxUsers,
    maxGoogleCalendars: overrides?.maxGoogleCalendars ?? p.maxGoogleCalendars,
  };
}
