...
const SYSTEM_CATEGORIES = [
  {
    ...
    modulesJson: JSON.stringify([
      "guests_full", "menu", "schedule", "budget", "seating",
      "vendors", "photos", "tasks", "finances", "board_links",
      "moodboard", "invitations", "wedding_website", "guest_portal",
      "ai_planner", "gifts",
    ]),
  },
  {
    ...
    isSystem: true,
    modulesJson: JSON.stringify([
      "guests_allergens", "menu", "schedule", "photos", "tasks",
    ]),
    defaultsJson: JSON.stringify({ guestListMode: "ALLERGENS_ONLY" }),
  },
  {
    name: "Wigilia",
    icon: "Snowflake",
  ...
  },
  {
    ...
    isSystem: true,
    modulesJson: JSON.stringify([
      "guests_full", "menu", "schedule", "budget", "tasks", "board_links",
    ]),
  },
  {
    name: "Inne",
...
export async function seedSystemCategories(organizationId: string) {
  const user = await getCurrentUser();
  if (!user) throw new Error("Unauthorized");

  const existing = await prisma.eventCategory.count({
    where: { organizationId, isSystem: true },
  });
  if (existing > 0) return { ok: true, message: "Kategorie systemowe już istnieją" };
