...
describe("effectiveLimits", () => {
  it("bierze limity z planu, gdy nie ma nadpisania", () => {
    expect(effectiveLimits("START")).toEqual({ maxAdmins: 1, maxUsers: 3 });
    expect(effectiveLimits("PRO")).toEqual({ maxAdmins: 2, maxUsers: 10 });
  });

  it("ENTERPRISE nie ma limitu", () => {
    expect(effectiveLimits("ENTERPRISE")).toEqual({ maxAdmins: null, maxUsers: null });
  });

  it("ręczne ustawienie per przestrzeń wygrywa z planem", () => {
    expect(effectiveLimits("START", { maxAdmins: 5, maxUsers: 25 })).toEqual({
      maxAdmins: 5,
      maxUsers: 25,
    });
  });
  ...
  it("nadpisanie tylko jednej wartości zostawia drugą z planu", () => {
    expect(effectiveLimits("PRO", { maxUsers: 40 })).toEqual({ maxAdmins: 2, maxUsers: 40 });
  });
  ...
  it("null w nadpisaniu znaczy „użyj limitu z planu”, nie „bez limitu”", () => {
    ...
    expect(effectiveLimits("START", { maxAdmins: null, maxUsers: null })).toEqual({
      maxAdmins: 1,
      maxUsers: 3,
    });
  });
