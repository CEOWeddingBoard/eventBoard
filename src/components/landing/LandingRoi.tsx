  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-sm font-bold tabular-nums text-stone-900">{display}</span>
      </div>
      <input
        type="range"
        min={min}
        ...
        className="mt-2 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-stone-200 accent-[#b45309]"
      />
      {hint && <p className="mt-1.5 text-[11.5px] leading-snug text-stone-500">{hint}</p>}
    </div>
  );
}
    ...
    <section id="kalkulator" className="bg-white py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#7a5f28]">Kalkulator</p>
        ...
        <div className="mx-auto mt-14 grid max-w-5xl gap-6 lg:grid-cols-[1.05fr_.95fr]">
          {/* Inputs */}
          <div className="rounded-2xl border border-stone-200/80 bg-[#fefdfb] p-6 shadow-sm sm:p-8">
            <h3 className="text-base font-bold text-stone-900">Twój obiekt</h3>
            <p className="mt-1 text-sm text-stone-500">Podaj przybliżone wartości — wynik liczy się od razu.</p>
            ...
            <div className="mt-6 space-y-6">
              ...
              <div>
                <label className="text-sm font-semibold text-stone-700">Plan EventBoard</label>
                <div className="mt-2 inline-flex gap-1 rounded-full bg-stone-100 p-1">
                  {plans.map((p) => (
                    <button
                      key={p.monthly}
                      type="button"
                      ...
                      className={`rounded-full px-4 py-1.5 text-sm font-semibold transition-colors ${
                        plan === p.monthly ? "bg-[#0f172a] text-white" : "text-stone-600 hover:text-stone-900"
                      }`}
                    >
                      {p.name}
                    </button>
          ...
          {/* Result */}
          <div className="flex flex-col gap-4 rounded-2xl border border-stone-200/80 bg-[#fefdfb] p-6 shadow-sm sm:p-8">
            <div
              className={`rounded-xl border p-5 ${
                net >= 0 ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"
              }`}
            >
              <p className={`text-xs font-bold uppercase tracking-wider ${net >= 0 ? "text-emerald-700" : "text-amber-700"}`}>
                Zysk netto miesięcznie
              </p>
              <p className="mt-1 text-4xl font-extrabold tracking-tight text-stone-900 sm:text-5xl">
                {net >= 0 ? `+${zl(net)}` : zl(net)}
              </p>
            ...
            <div>
              <div className="flex items-center justify-between text-xs font-semibold text-stone-600">
                ...
                <span className="text-stone-500">{totalHours} h odzyskane / mies.</span>
              </div>
        ...
        <p className="mx-auto mt-8 max-w-2xl text-center text-xs text-stone-500">
...
function Stat({ icon, k, v, muted }: { icon: React.ReactNode; k: string; v: string; muted?: boolean }) {
  return (
    <div className="rounded-xl bg-stone-50 p-4">
      ...
      <p className={`mt-1 text-xl font-extrabold tabular-nums ${muted ? "text-stone-600" : "text-stone-900"}`}>{v}</p>
    </div>
  );
}
