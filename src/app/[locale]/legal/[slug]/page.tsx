...
export default async function LegalDocPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-12">
      ...
      <pre className="mt-6 whitespace-pre-wrap rounded-xl border border-neutral-200 bg-white p-6 font-sans text-sm leading-relaxed text-neutral-700">
        {doc.body}
      </pre>
      <div className="mt-8 flex flex-wrap gap-4 border-t border-neutral-200 pt-4 text-sm">
        {LEGAL_DOCS.map((d) => (
          <Link key={d.slug} href={`/${locale}/legal/${d.slug}`} className="text-neutral-600 underline-offset-4 hover:text-neutral-900 hover:underline">
            {d.title}
          </Link>
        ))}
      </div>
